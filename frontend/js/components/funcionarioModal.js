...
export function criarFuncionarioModal() {
    ...
    modal.addEventListener('show.bs.modal', async function (e) {
        // Evento de clique em um card de funcionario
        // document.addEventListener('click', async (e) => {
        // Verifica se o click foi em um card de funcionario
        const card = e.relatedTarget;

        if (!card) {
            console.log('Clique fora do card de funcionario'); // debug
            return;
        }

        // Obtém o cpf do funcionario clicado
        const cpf_func = card.id;
        console.log('CPF do Funcionário:', cpf_func); // debug
        ...
        // Itera sobre as entradas do funcionario
